...
namespace ex_mib_map
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //1
            products
                .Select(p => (p.Producer.Insert(3, "...").Insert(6, p.Producer.Last().ToString()).Remove(7), i18n[p.ProductName], p.PricePerUnit * p.Quantity))
                .ToList()
                .ForEach(p => Console.WriteLine(p));

            //1.2
            string fileName = "export.csv";
            File.WriteAllText(fileName, "Seller,Product,CA\n");
            ...
            Func<int, string> CategorizeProductStock = q =>
            {
                ...
                if (q > 15) return "Stock élevé";
                return "Stock normal";
            };

            Func<double, string, double> MajoratePrice = (p, q) =>
            {
                if (q == "Stock faible") return p + p * 0.15;
                if (q == "Stock normal") return p + p * 0.05;
                return p;
            };
            ...
            products
                .Select(p => (String.Concat(p.Producer.First(), p.Producer.Length.ToString(), p.Producer.Last()), CategorizeProductStock(p.Quantity), MajoratePrice(p.PricePerUnit, CategorizeProductStock(p.Quantity)) * p.Quantity > 100 ? "Premium" : "Standard"))
                .ToList()
                .ForEach(p => ExportJson(p));
        }
    }
}
